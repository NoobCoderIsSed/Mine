...
class GitHubMinerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GitHub Profile Miner - Auto Discovery")
        ...
        # Progress frame (shared across tabs)
    ...
    def setup_manual_tab(self):
        """Setup the manual input tab (original functionality)."""
        self.manual_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.manual_tab, text="Manual Input")
        
        # Token input
        ttk.Label(self.manual_tab, text="GitHub Token:").pack(pady=5)
        self.entry_token = ttk.Entry(self.manual_tab, width=50, show="*")
        self.entry_token.pack(pady=5)
        
        # Profile URL input
        ttk.Label(self.manual_tab, text="GitHub Profile URL:").pack(pady=5)
        self.entry_url = ttk.Entry(self.manual_tab, width=50)
        ...
        ttk.Button(button_frame, text="Mine Single Profile", command=self.start_manual_mining).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Set Global Token", command=self.set_global_token).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Convert JSON to CSV", command=self.convert_json_to_csv).pack(side=tk.LEFT, padx=5)
    
    def setup_auto_discovery_tab(self):
        """Setup the automatic discovery tab."""
    ...
    def mine_discovered_users(self, usernames, output_prefix):
        ...
        try:
            ...
            for i in range(0, len(usernames), batch_size):
                ...
                try:
                    ...
                    if batch_results:
                        ...
                        intermediate_file = f"{output_prefix}_batch_{i//batch_size + 1}_{timestamp}"
            ...
            if all_results and not self.stop_event.is_set():
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                final_file = f"{output_prefix}_final_{timestamp}"
                ...
                self.update_status(f"Final output: {final_file}")
    ...
    def start_manual_mining(self):
        """Start manual mining for a single profile."""
        self.stop_event.clear()
        self.stop_button.config(state='normal')
        self.progress_bar.start()
        ...
        self.mining_thread = threading.Thread(target=self.manual_mining_worker)
        self.mining_thread.daemon = True
        self.mining_thread.start()
    
    def manual_mining_worker(self):
        """Worker for manual single profile mining."""
        try:
    ...
    def set_global_token(self):
        ...
        if not token or token.strip() == "":
            ...
            return
        GITHUB_TOKEN = token
        # Also set it in the auto discovery tab
        self.auto_token_entry.delete(0, tk.END)
